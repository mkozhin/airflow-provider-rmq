...
- **Added:** A `conn_id` whose tasks are alive but never reach `listening` for two cycles in a row has its connection recreated on that basis alone — a pooled connection whose `channel()` never returns leaves the check with no candidate to judge, while every task retrying on it meets the same silence. A `conn_id` no check has ever reached a verdict on is reported as `unknown` rather than green
...
- **Added:** Separate publishing connection per `conn_id` for cooldown publishing. Under a resource alarm RabbitMQ blocks a publishing connection by no longer reading from its socket, which takes down `basic.ack` of every consumer sharing it; with publishing split off, the block stays inside the publishing connection. Two publish timeouts in a row are its own liveness verdict and recreate it alone
