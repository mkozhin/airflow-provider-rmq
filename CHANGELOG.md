...
- **Added:** Liveness watchdog — every reconcile cycle asks the broker whether the watcher's consumers are still registered, looking for each subscription's own consumer tag `rmq_watcher.{hostname}.{pid}.{sub_id}.{nonce}` in `GET /api/consumers/{vhost}`, or falling back to a passive `queue_declare` probe when the connection has no `management_url`. Two negative checks in a row cancel exactly the subscriptions whose own tag the broker did not confirm, drop the connection they share and start fresh tasks — restarting a task alone would hand it the same zombie connection out of the pool. The confirmed subscriptions of that `conn_id` are neither cancelled nor restarted: they reconnect through their own retry loop and surface the drop as a transient consumer error, with a WARNING naming them; the cooldown fire task goes down with the connection it runs on, because it holds that connection object for its whole life. A Management API failure counts as "no data": WARNING, counters untouched, stored status kept. The two consecutive checks gate the restart, not the report: a `conn_id` whose consumers the broker denies is written as `error` with the reason and the check number on the cycle that finds it, rather than reading `connected` for the interval the second check waits out. The same `conn_id` is recreated at most once every 5 cycles
...
- **Added:** Columns `broker_consumer_count` and `last_reconcile_at` in `rmq_watcher_conn_status`, created by an idempotent migration (`inspect` + `ALTER TABLE ... ADD COLUMN` for what is missing) that the watcher loop retries once per cycle, one attempt in flight with a growing backoff, until it succeeds — a database unavailable at plugin load no longer leaves the process running against an outdated table
...
- **Added:** Blocking calls run in bounded thread pools of the watcher's own — one for consumer tasks, one for the reconcile cycle, never shared: a database stuck on consumer work must not starve the cycle that is supposed to notice and recover from it. Pool exhaustion is logged rather than silently queued
...
- **Added:** Statsd metrics `rmq_watcher.consumer_reconnect`, `rmq_watcher.consumer_restarted`, `rmq_watcher.cycle_timeout`, `rmq_watcher.dag_triggered` (no-ops when statsd is not configured)
