...
- **Added:** Liveness watchdog — every reconcile cycle asks the broker whether the watcher's consumers are still registered, looking for each subscription's own consumer tag `rmq_watcher.{hostname}.{pid}.{sub_id}` in `GET /api/consumers/{vhost}`, or falling back to a passive `queue_declare` probe when the connection has no `management_url`. Two negative checks in a row cancel the tasks of that `conn_id` (the cooldown fire task included), drop the connection and start fresh ones — restarting a task alone would hand it the same zombie connection out of the pool. A Management API failure counts as "no data": WARNING, counters untouched, stored status kept. The same `conn_id` is recreated at most once every 5 cycles
...
- **Added:** Log lines the watcher was missing — an INFO on a successfully started thread (with the cycle interval and budget), an INFO on every successful (re)connection with `conn_id` and queue name, and an explicit reason when `on_starting` does not recognise the Airflow component and leaves the watcher unstarted
...
- **Changed:** `status` in `rmq_watcher_conn_status` follows the broker's verdict instead of the number of unfinished tasks, and a row is written for every `conn_id` present in the subscription list — a `conn_id` whose tasks all died used to freeze on its last value, which is how a dead connection kept showing `connected` for a day
