...
def read_positive(name: str, cast: Callable[[str], Any]) -> Any:
    ...
    :returns: The value, or ``None`` when there is no usable one: the Variable is unset,
        holds something that is not a number, or holds a number at or below zero. Both
        readers take ``None`` as "keep the built-in default", and the two unusable cases
        are logged as they are found.
