...
- **Flow control покрывается таймаутом на `publish` плюс явным различением, а не
  колбэком `blocked`.** Публичных колбэков `connection.blocked`/`unblocked` в
  aio-pika 9.x нет: blocked-фреймы обрабатываются внутри aiormq через приватный
  event. Но одного таймаута мало: под memory/disk alarm брокер перестаёт читать
  из сокета целиком, поэтому зависает и passive-проба живости — и watchdog принял
  бы блокировку за смерть, снося соединение каждые два цикла, пока держится
  alarm. Различаем по факту: если на этом `conn_id` недавно (в пределах текущего
  и предыдущего цикла) `publish` отвалился по таймауту, зависшая проба
  трактуется как flow control — статус `degraded`, счётчик отрицательных проверок
  не растёт, соединение не пересоздаётся. Сообщения при этом не теряются: они не
  подтверждены и обработаются после снятия блокировки.
...
- Флаг «недавний таймаут `publish`» хранится на уровне `conn_id` в менеджере и
  живёт два цикла: он отличает flow control брокера от мёртвого соединения при
  разборе зависшей пробы живости.
...
- Для каждого `conn_id` с активными подписками:
  - есть `management_url` → один запрос очередей vhost'а с `consumer_details`;
    подписка жива, если её собственный `consumer_tag` присутствует в ответе;
...
- **Классификация исходов проверки:**
  ...
  | AMQP-проба зависла, а `publish` на этом `conn_id` недавно отвалился по таймауту | flow control | статус `degraded`, счётчик не трогать |
  | AMQP-проба упала или зависла в прочих случаях | мертва | +1 к счётчику |
...
- Каждый `run_in_executor` в consumer-ветках (`_sync_trigger`, запись статуса)
  ...
  осталась бы `listening` при неработающем потреблении и заодно держала бы
  воркер общего пула. Таймаут превращает это в обычный отказ триггера: NACK,
  backoff, статус `error`.
...
- **Неудачная миграция повторяется, а не проглатывается.** `ensure_table_exists`
  ...
  каждой итерации вызывает `ensure_table_exists()` заново, пока флаг не станет
  «выполнено» — сам по себе флаг никто бы не перечитал, потому что единственный
  существующий колл-сайт `plugin.py:38` отрабатывает один раз за жизнь процесса.
...
- [ ] запоминать на уровне `conn_id` факт недавнего таймаута `publish` (живёт два цикла) — признак flow control для разбора зависшей пробы живости
...
- [ ] в начале каждой итерации `_run_cycle` вызывать `ensure_table_exists()`, пока миграция не отмечена выполненной — это и есть недостающий колл-сайт повтора из Task 3
...
- [ ] ветка с `management_url`: запрашивать `consumer_details` и искать собственный `consumer_tag` подписки; ни `consumers > 0`, ни сравнение с числом локальных тасок не годятся — вторая HA-реплика маскирует нашего зомби своим живым consumer'ом
...
- [ ] отличать flow control от смерти: зависшая проба при недавнем таймауте `publish` на этом `conn_id` → статус `degraded`, счётчик не растёт, соединение не пересоздаётся
...
- [ ] тест: зависшая проба после таймаута `publish` → `degraded`, перезапуска нет (регрессия на memory alarm)
...
- [ ] тест: зависший `_sync_trigger` отваливается по своему таймауту → NACK с backoff, статус `error`, воркер освобождается
...
- [ ] зафиксировать, почему `set_qos` не задаётся вовсе: при requeue несовпавших любой конечный prefetch рано или поздно останавливает потребление
- [ ] зафиксировать, как flow control отличается от смерти соединения и почему это различение обязательно
