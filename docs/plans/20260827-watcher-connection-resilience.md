...
- **Flow control отдельного механизма не требует.** Под resource alarm RabbitMQ
  блокирует только **публикующие** соединения: потребляющее продолжает получать
  доставки и отвечать на запросы. Publish у watcher'а есть лишь в cooldown-ветке
  (`consumer.py:616-623`), поэтому в основном режиме alarm вообще не мешает
  проверке живости — наш `consumer_tag` остаётся в ответе Management API,
  проверка положительна, соединение не пересоздаётся. Читать `mem_alarm` из
  `GET /api/nodes` не нужно и вредно: гейт «alarm → не трогать счётчик» глушил бы
  watchdog для всех `conn_id` на всё время алярма кластера (включая соединения,
  ставшие зомби именно тогда), а сам эндпоинт закрыт проверкой уровня
  `monitoring`, которого документированный контракт подключения
  (`readme.md:657-665`) не требует. Зависший `publish` под alarm остаётся обычной
  транзиентной ошибкой: сообщение не подтверждено и обработается после снятия
  блокировки.
...
- **Неудачная миграция повторяется, а не проглатывается.** `ensure_table_exists`
  ...
  собственного таймаута фаза `migrate` выбирала бы весь бюджет цикла, а
  `finally: await self._manager.stop()` сносил бы потребителей каждые несколько
  минут, пока держится блокировка.
...
- [ ] хранить на уровне `conn_id` время последнего `_drop_connection` — ограничитель частоты пересозданий для конфигураций без `management_url`, где alarm определить нечем
...
- [ ] зафиксировать разделение пулов executor'а и что таймаут вокруг `run_in_executor` не освобождает воркер
...
- [ ] проверить, что memory alarm брокера не приводит ни к пересозданиям соединений, ни к заморозке watchdog'а: подписки без publish продолжают работать, cooldown-публикации отваливаются по таймауту и повторяются
