...
- [ ] в начале каждой итерации `_run_cycle` вызывать `ensure_table_exists()` **через executor и под собственным коротким `wait_for`**, пока миграция не отмечена выполненной — это недостающий колл-сайт повтора из Task 3; прямой синхронный вызов заблокировал бы петлю и обошёл собственный cycle timeout, а без своего таймаута `ALTER TABLE`, стоящий за ACCESS EXCLUSIVE, выбирал бы весь бюджет цикла
- [ ] таймаут или ошибка миграции → WARNING, флаг не выставляется, цикл продолжается
- [ ] попытка миграции единственная в полёте, между неудачными — растущий backoff (цикл → удвоение → потолок порядка часа): таймаут не освобождает воркер, поэтому повтор каждый цикл исчерпал бы пул цикла застрявшими попытками и удерживал бы соединения общего движка Airflow
- [ ] писать текущую фазу цикла в атрибут перед каждым шагом (migrate / scan / sync / read subs / reconcile)
...
- [ ] завести два собственных `ThreadPoolExecutor` вне event loop — для цикла и для consumer-веток — и использовать их во всех `run_in_executor`; общий пул давал бы каскад: зависшие `_sync_trigger` съедают воркеры, за ними встают операции цикла, петля пересоздаётся и виснет снова
- [ ] `shutdown_default_executor()` не применять — на Python 3.10/3.11 он завершается `thread.join()` без таймаута, который `wait_for` не отменяет, и при зависшей БД вешает петлю намертво
- [ ] логировать исчерпание любого из пулов
...
- [ ] тест: невыполненная миграция повторяется на следующем цикле и перестаёт вызываться после успеха
...
- [ ] тест: пересоздание петли переиспользует те же пулы и не вызывает `shutdown_default_executor`
- [ ] тест: занятый consumer-пул не мешает операциям цикла (изоляция пулов)
- [ ] тест: зависшая миграция отваливается по своему таймауту, не выбирая бюджет цикла
- [ ] тест: пока попытка миграции в полёте, новая не запускается — число застрявших воркеров не растёт с числом циклов
- [ ] тест: зависшая задача в executor'е не мешает пересозданию петли
