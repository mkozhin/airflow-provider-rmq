...
- **Живость определяется расхождением счётчиков, а не признаком `consumers > 0`.**
  Management API возвращает суммарное число consumer'ов очереди, а на одной
  очереди законно сидят несколько потребителей: ADR-0002 фиксирует NACK
  несовпавших для смешанного трафика, уникальный ключ по тройке
  (dag_id, queue_name, conn_id) разрешает двум DAG'ам одну очередь, а HA-режим
  документирован в `readme_ru.md:735`. Отрицательной проверка считается, когда
  брокер видит **меньше** consumer'ов, чем у нас живых тасок по этой очереди.
...
- **Flow control покрывается таймаутом на `publish`, а не статусом `blocked`.**
  Публичных колбэков `connection.blocked`/`unblocked` в aio-pika 9.x нет:
  blocked-фреймы обрабатываются внутри aiormq через приватный event. Обещать
  статус `blocked` значило бы либо получить `AttributeError` при настройке
  соединения, либо тихо не реализовать пункт. Защита остаётся полной и без него:
  `publish` под `rpc`-таймаутом не виснет, сообщение не подтверждается и
  возвращается в очередь. Отдельный `blocked`-таймаут поэтому не заводится.
- **prefetch задаётся с запасом, а не равным 1.** RabbitMQ возвращает
  requeue-нутое сообщение в голову очереди, а несовпавшие по фильтру именно
  requeue-ются (ADR-0002). При `prefetch=1` брокер не отдал бы следующее
  сообщение до подтверждения текущего, и consumer вечно получал бы одно и то же
  несовпавшее, никогда не доходя до подходящего за ним: на classic-очереди —
  бесконечный цикл, на quorum — dead-letter за пару секунд. Prefetch нужен только
  чтобы ограничить окно неподтверждённых после перехода на ACK-после-триггера,
  и для этого достаточно значения порядка 50.
...
- `asyncio.wait_for(..., timeout=timeouts.rpc)` вокруг всех AMQP-RPC:
  ...
  транзиентная ошибка.
- `channel.set_qos(prefetch_count=_PREFETCH_COUNT)` (константа = 50) сразу после
  открытия канала в consumer-ветках: после перехода на ACK-после-триггера
  неограниченный prefetch означал бы лавину редоставок при обрыве. Значение
  заведомо больше 1 — при `prefetch=1` requeue несовпавшего сообщения
  заблокировал бы голову очереди навсегда (см. Ключевые решения).
...
- Flow control (`connection.blocked`) отдельного обработчика не получает:
  публичных колбэков в aio-pika 9.x нет, а `publish` под `rpc`-таймаутом и так
  не виснет — сообщение остаётся неподтверждённым и возвращается в очередь.
...
- Для каждого `conn_id` с активными подписками:
  - есть `management_url` → один запрос списка очередей vhost'а; по каждой
    очереди сравнивается `consumers` из ответа с числом наших живых тасок,
    подписанных на эту очередь через этот `conn_id`;
...
- **Классификация исходов проверки:**
  ...
  |---|---|---|
  | `consumers` ≥ числа наших живых тасок по очереди | жива | сброс счётчика |
  | `consumers` < числа наших живых тасок по очереди | мертва | +1 к счётчику |
  ...
  Сравнение идёт по расхождению, а не по признаку `consumers > 0`: на одной
  очереди законно сидят чужие потребители и реплики, поэтому ненулевой суммарный
  счётчик не доказывает, что жив именно наш consumer. Расхождение адресуется
  всем подпискам этой очереди — какая именно из наших тасок отвалилась, брокер
  не сообщает.
...
- Fire-очередь проверяется на живость наравне с очередями подписок: она не
  входит в список подписок, поэтому её имя добавляется в проверку отдельно,
  когда fire-таска запущена.
...
- **Неудачная миграция повторяется, а не проглатывается.** `ensure_table_exists`
  ...
  результат миграции запоминается флагом, и при неудаче попытка повторяется в
  начале reconcile-цикла, пока не пройдёт; чтение статусов во вью оборачивается
  `try/except` с сообщением «схема не обновлена» вместо ошибки рендеринга.
...
### Task 2: Таймауты AMQP-операций, prefetch и состояние подписки в менеджере
...
- [ ] добавить `channel.set_qos(prefetch_count=_PREFETCH_COUNT)` (константа = 50) после открытия канала в consumer-ветках; значение заведомо больше 1, иначе requeue несовпавшего сообщения блокирует голову очереди
...
- [ ] тест: `set_qos` вызывается с ожидаемым prefetch (> 1)
...
- [ ] запоминать результат миграции флагом и повторять попытку, пока она не пройдёт (однократный вызов из `on_load` оставляет процесс с моделью, знающей колонки, при таблице без них)
...
- [ ] ветка с `management_url`: сравнивать `consumers` по очереди с числом наших живых тасок на ней; отрицательная проверка — когда брокер видит меньше (признак `consumers > 0` не годится: на очереди законно сидят чужие потребители и реплики)
- [ ] включать в проверку очередь `rmq_watcher.fire`, когда fire-таска запущена — в списке подписок её нет
...
- [ ] тест: брокер видит меньше consumer'ов, чем у нас тасок, дважды подряд → подписка в наборе на перезапуск
- [ ] тест: две подписки на одной очереди и `consumers = 1` → это отрицательная проверка, а не «жива» (регрессия на чужих потребителей)
- [ ] тест: `consumers` больше числа наших тасок (чужой потребитель) → подписка считается живой
...
- [ ] `_update_all_conn_counts`: определять `status` результатом проверки живости — `connected` при подтверждении, `error` при отрицательной проверке, сохранение прежнего при «нет данных»
...
- [ ] зафиксировать, почему живость меряется расхождением счётчиков, а не признаком `consumers > 0` (на очереди законно несколько потребителей)
- [ ] зафиксировать, почему время цикла пишется явной колонкой, а не берётся из `updated_at` с `onupdate`
- [ ] зафиксировать отказ от статуса `blocked` в пользу таймаута на `publish`, отказ от `shutdown_default_executor` в пользу собственного пула и почему TCP keepalive остался вне скоупа
...
- [ ] проверить, что несовпавшее по фильтру сообщение не останавливает потребление при включённом prefetch
- [ ] проверить, что сброс соединения поднимает заново и fire-таску

### Task 13: [Final] Update documentation

- [ ] CHANGELOG: heartbeat по умолчанию, таймауты AMQP-операций, prefetch, watchdog подписок и цикла, at-least-once в immediate-режиме, колонки `broker_consumer_count` и `last_reconcile_at`
...
- проверить flow control: перевести брокер в состояние memory alarm и убедиться,
  ...
  возвращается в очередь и обрабатывается после снятия блокировки
