...
- **Flow control опознаётся у брокера, а не выводится из поведения клиента.**
  Публичных колбэков `connection.blocked`/`unblocked` в aio-pika 9.x нет:
  blocked-фреймы обрабатываются внутри aiormq через приватный event. Одного
  таймаута на `publish` тоже мало: под memory/disk alarm брокер перестаёт читать
  из сокета целиком, поэтому зависает и passive-проба живости — и watchdog
  принял бы блокировку за смерть. Выводить alarm из «недавно был таймаут
  `publish`» нельзя: зависший `publish` означает ровно то же, что зависшая
  проба, и различить по нему два состояния невозможно. Поэтому alarm читается у
  брокера: `GET /api/nodes` отдаёт `mem_alarm` и `disk_free_alarm`. При активном
  alarm статус `degraded`, счётчик отрицательных проверок не растёт, соединение
  не пересоздаётся; сообщения не теряются — они не подтверждены и обработаются
  после снятия блокировки.
- **Без `management_url` alarm не определяется, поэтому ограничивается частота
  пересозданий.** В этой конфигурации отличить блокировку от смерти нечем, и
  единственная защита от цикла пересозданий — не сносить одно и то же соединение
  чаще, чем раз в N циклов (N порядка 5), с эскалацией в лог при каждом
  срабатывании ограничителя. Ограничение сохраняет восстановление после реального
  разрыва (оно происходит один раз), но превращает цикл под alarm в редкое
  событие вместо непрерывного.
...
- Состояние alarm читается раз в цикл на `conn_id` с `management_url` и
  кешируется на время цикла: под alarm это единственный запрос, который брокер
  гарантированно обслуживает, тогда как AMQP-операции по заблокированному
  соединению стоят.
- Для `conn_id` без `management_url` менеджер хранит время последнего
  `_drop_connection` и не пересоздаёт то же соединение чаще, чем раз в N циклов.
...
- **Классификация исходов проверки:**
  ...
  | брокер сообщает активный alarm (`/api/nodes`) | flow control | статус `degraded`, счётчик не трогать |
  | AMQP-проба упала или зависла, alarm неизвестен или отсутствует | мертва | +1 к счётчику |

  Строка про alarm проверяется **до** остальных и одинаково в обеих ветках:
  иначе под memory alarm ветка с `management_url` продолжала бы находить тег и
  писать `connected`, а обещанный `degraded` не наступал бы никогда.
...
- Каждый `run_in_executor` в consumer-ветках (`_sync_trigger`, запись статуса)
  ...
  вызова. При устойчиво недоступной БД пул исчерпывается, и остальные операции
  выстраиваются в очередь за ним. Это принимается: watcher без БД всё равно не
  может ни запустить DAG, ни записать статус, а backoff в ветке отказа триггера
  ограничивает скорость появления новых зависших вызовов. `max_workers` берётся с
  запасом относительно числа подписок, а исчерпание пула логируется — молча
  деградировать оно не должно.
...
- **Неудачная миграция повторяется, а не проглатывается.** `ensure_table_exists`
  ...
  обслуживается заблокированной петлёй, — а при blackhole-недоступности БД
  зависание длилось бы до TCP-таймаута ОС, попутно остановив отправку AMQP
  heartbeat'ов из той же петли.
...
- [ ] в начале каждой итерации `_run_cycle` вызывать `ensure_table_exists()` **через executor**, пока миграция не отмечена выполненной — это недостающий колл-сайт повтора из Task 3, а прямой синхронный вызов заблокировал бы петлю и обошёл собственный cycle timeout
...
- [ ] завести собственный `ThreadPoolExecutor` с ограниченным `max_workers` вне event loop и использовать его во всех `run_in_executor`; `shutdown_default_executor()` не применять — на Python 3.10/3.11 он завершается `thread.join()` без таймаута, который `wait_for` не отменяет, и при зависшей БД вешает петлю намертво
...
- [ ] тест: пересоздание петли переиспользует тот же executor и не вызывает `shutdown_default_executor`
...
- [ ] отличать flow control от смерти по данным брокера: `GET /api/nodes` → активный `mem_alarm`/`disk_free_alarm` → статус `degraded`, счётчик не растёт, соединение не пересоздаётся; проверка alarm идёт до остальных исходов и в обеих ветках
- [ ] для `conn_id` без `management_url` (alarm определить нечем) ограничить частоту `_drop_connection` — не чаще раза в N циклов, с логом при каждом срабатывании ограничителя
...
- [ ] тест: активный alarm у брокера → `degraded`, перезапуска нет, даже когда тег находится (регрессия на memory alarm)
- [ ] тест: без `management_url` повторные отрицательные проверки не пересоздают соединение чаще ограничителя
...
- [ ] `_update_all_conn_counts`: определять `status` результатом проверки живости — `connected` при подтверждении, `error` при отрицательной проверке, `degraded` при flow control, сохранение прежнего при «нет данных»
...
- [ ] тест: flow control → статус `degraded`, соединение не пересоздаётся
...
- [ ] `_run_cycle`: `_scan_subscriptions` и `_sync_to_db` через `run_in_executor` с собственным пулом из Task 4 (не `None`)
...
- [ ] обернуть `wait_for` каждый `run_in_executor` в consumer-ветках (`_sync_trigger`, запись статуса): их ждёт consumer-таска, а не `_run_cycle`, поэтому cycle timeout их не покрывает — зависший вызов оставлял бы таску `listening` при неработающем потреблении и держал воркер общего пула
...
- [ ] зафиксировать, как flow control отличается от смерти соединения (alarm читается у брокера, а не выводится из поведения клиента) и что при недоступном Management API различения нет — вместо него ограничитель частоты пересозданий
...
- [ ] проверить, что memory alarm брокера даёт `degraded`, а не цикл пересозданий соединения каждые два интервала
...
- проверить flow control: перевести брокер в состояние memory alarm и убедиться,
  что consumer-таска не виснет (`publish` отваливается по таймауту, сообщение
  возвращается в очередь), статус становится `degraded`, а соединения при этом
  **не** пересоздаются каждые два интервала
