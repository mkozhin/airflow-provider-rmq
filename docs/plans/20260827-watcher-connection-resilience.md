...
- [ ] расширить ключ пула соединений до `(conn_id, role)` (`consume` / `publish`) и выполнять cooldown-публикации на отдельном лениво поднимаемом publish-соединении: сейчас все подписки `conn_id` делят одно соединение с публикующим каналом, и блокировка брокером под alarm остановила бы `basic.ack` всех потребителей этого `conn_id`
- [ ] перекейовать **все** обращения к `self._connections`, а не только в `_get_or_create_connection`/`_drop_connection`: `consumer.py:198` (сравнение ключей пула с множеством строк `active_conn_ids` — иначе каждый `reconcile` закрывает все соединения) и `consumer.py:217` (получение соединения для fire-таски — иначе она не стартует никогда)
- [ ] считать два подряд таймаута `publish` на `conn_id` отрицательным вердиктом для publish-роли и сбрасывать `_drop_connection(conn_id, role="publish")`: у publish-соединения нет потребителей, поэтому watchdog Task 5 его не видит и зомби на нём не лечится ничем
- [ ] при ошибке `publish` явно NACK-ать текущую доставку: выход из итератора возвращает только то, что осталось в его внутренней очереди, а извлечённое сообщение иначе висит до `consumer_timeout`
- [ ] добавить `_drop_connection(conn_id)` — best-effort `close()` под коротким `wait_for` + удаление из кеша обеих ролей
...
- [ ] подписываться с детерминированным `consumer_tag` (`rmq_watcher.{hostname}.{pid}.{sub_id}`, для fire-таски — `.fire`), чтобы проверка живости могла опознать своего consumer'а среди чужих
- [ ] хранить на уровне `conn_id` время последнего `_drop_connection` — на нём строится ограничитель частоты пересозданий, который Task 5 применяет ко всем `conn_id` без исключений
- [ ] `set_qos` **не** добавлять: любой конечный prefetch останавливает потребление, когда несовпавших по фильтру накапливается на размер окна; отсутствие QoS зафиксировать комментарием, чтобы его не «починили» обратно
- [ ] перенести `_ConsumerState` из локальной переменной `_consume_subscription` в `_ActiveSub`, чтобы менеджер знал статус своих тасок (без этого гейт watchdog'а в Task 5 невычислим)
...
- [ ] тест: `_drop_connection` удаляет из кеша оба соединения `conn_id` даже когда `close()` зависает; с `role="publish"` — только publish-соединение
- [ ] тест: cooldown-публикация идёт на publish-соединении, а потребление — на consume-соединении того же `conn_id`
- [ ] тест: `reconcile` с активными подписками не закрывает их соединения (регрессия на сравнение кортежных ключей со строками)
- [ ] тест: fire-таска стартует после `_provision_cooldown` (регрессия на получение соединения по строковому ключу)
- [ ] тест: два подряд таймаута `publish` пересоздают только publish-соединение, consume остаётся тем же
- [ ] тест: отказ `publish` возвращает текущее сообщение в очередь немедленно, а не оставляет неподтверждённым
...
- [ ] тест: `consumer_tag` детерминирован и различает подписки одного процесса
- [ ] тест: серия несовпавших по фильтру сообщений не останавливает потребление — совпавшее за ними обрабатывается
- [ ] тест: статус подписки читается из `_ActiveSub` после старта таски и меняется при переходе `connecting` → `listening`
