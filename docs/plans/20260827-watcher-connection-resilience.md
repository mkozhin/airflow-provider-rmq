...
- **Flow control сверх этого отдельного механизма не требует.** С разведёнными
  ...
  Зависший `publish` под alarm остаётся обычной транзиентной ошибкой: сообщение
  не подтверждено и обработается после снятия блокировки.
...
- [ ] тест: `_drop_connection` удаляет из кеша оба соединения `conn_id` даже когда `close()` зависает
...
- [ ] зафиксировать разведение publish и consume по разным соединениям и почему общее соединение делает alarm проблемой всех подписок `conn_id`
