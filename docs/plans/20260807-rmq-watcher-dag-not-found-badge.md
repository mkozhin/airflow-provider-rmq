...
- [ ] добавить двухстрочный комментарий в самой `get_active_dag_ids` — что `is_paused` не фильтруется намеренно, со ссылкой **на ADR 0006** и на само поведение («paused DAG намеренно попадает в результат»). Имя теста в комментарий **не** вписывать: имена тестов дрейфуют, а протухшая ссылка в проде хуже её отсутствия — если пойнтер на тест нужен, его место в ADR, где он дёшево обновляется
- [ ] в том же ADR — строка про поведение при будущем переименовании колонки (Airflow 3 переименовывает флаги вроде `is_active`): запрос упадёт, сработает fail-open, бейдж просто исчезнет и в лог уйдёт WARNING. Деградация, а не поломка страницы — это и есть причина, по которой lookup обёрнут в `try/except`
