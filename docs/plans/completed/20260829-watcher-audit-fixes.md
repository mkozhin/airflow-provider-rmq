...
- `ruff` не должен получить новых замечаний относительно базы ветки (сейчас 135,
...
- [x] запись: замена `self._stop_event` в `_start()` (`listener.py:722`) сразу
      ...
      - ⚠️ две ссылки в теле записи проверку не проходили и исправлены:
        `self._manager` стоит на `listener.py:762`, `self._waker` — на `761`
...
- [x] проверить, что ни один путь из Overview не оставляет подписку с зелёным
      статусом при отсутствующем consumer'е
      - major 1, форма 1 из `__anext__` → `test_a_failed_cancel_of_the_consumer_
        itself_still_ends_the_task` (страж: путь остаётся под эвристикой)
      - major 1, таймаут публикации плейсхолдера (форма 2) →
        `test_a_publish_that_outran_its_bound_reports_error_and_retries`
      - major 1, «второй вход» — разрыв publish-соединения внутри
        `_get_publish_channel` (форма 3) →
        `test_a_publish_connection_dropped_mid_rpc_reports_error_and_retries`
      - major 1, `_DB_TIMEOUT` чтения Airflow-connection →
        `test_a_publish_connection_stuck_on_the_metadata_db_reports_error_and_retries`
      - major 2, склейка ответа Management API →
        `test_two_logins_on_one_vhost_each_get_their_own_answer` плюс два стража
      - находка «сверх отчёта», недописанный финальный статус →
        `test_the_next_cycle_writes_the_status_of_a_subscription_that_is_gone`,
        `test_two_failed_cycles_in_a_row_do_not_lose_the_status`,
        `test_a_cycle_cancelled_on_the_final_write_does_not_lose_it`,
        `test_the_pass_survives_a_row_that_is_no_longer_there`
      - ⚠️ уточнение к формулировке критерия: major 2 даёт ложный **отрицательный**
        вердикт (`degraded` при живом consumer'е), а не зелёный статус; путь
        закреплён тестом, но к «зелёному при отсутствующем consumer'е» относятся
        только major 1 и находка сверх отчёта
      - ⚠️ у пути `ConnectionError(... ) from None` (`consumer.py:1594`) нет
        отдельного сквозного теста: он закреплён юнит-тестом формы 2, который
        поднимает исключение с тем же текстом, а сквозной формы 2 идёт через
        `call_with_timeout`. Обе позиции — одна и та же ветка эвристики
- [x] проверить, что `_raised_while_cancelling` даёт ожидаемый ответ на всех
      пяти формах исключения из Overview
      - `TestTellingCancellationFromDeliveryFaults` — 6 тестов, все зелёные:
        форма 1 и форма 5 → отмена, форма 2 (в двух видах, в том числе из
...
- [x] проверить, что в `CHANGELOG.md` не появилось пунктов о дефектах, которых
      ...
      - по `git diff 9db910a..HEAD -- CHANGELOG.md`: пункты `**Fixed:**`
        изменились только в `v2.4.0` (25 → 14), во всех выпущенных версиях
        количество не тронуто. Из пяти добавленных строк `**Fixed:**` все пять —
        переписанные существующие пункты (три совпадают дословным началом, два
        переформулированы: пункт про паузу после чистого выхода итератора и
        пункт про сравнение только по статусу), остальные одиннадцать свёрнуты
        в `**Added:**`. Ни одного нового пункта не появилось
- [x] run full test suite: `pytest`
      - `1000 passed, 1 skipped` за 17.81 с
- [x] сверить `ruff` с базой ветки: новых замечаний нет
      - 135 сейчас, 135 на `9db910a` (замер во временном worktree), 146 на
        merge-base с `main`
      - ⚠️ в Development Approach стояло «сейчас 126» — число устарело и
        исправлено на 135 в рамках этой задачи
- [x] проверить, что `.revmux/` не попал ни в один коммит
      - девять коммитов ветки (`9711039..bfc660b`), `git log --name-only` не
        содержит ни одного пути с `revmux`; каталог закрыт `.gitignore:22`

### Task 11: [Final] Update documentation

- [x] сверить `readme.md` и `readme_ru.md` с изменившимся поведением
      - оба файла последний раз менялись коммитом `30c6bb3`, то есть ровно перед
        работой по этому плану, и правок не требуют. Проверены четыре места, в
        которые могли бы бить задачи 1–3 и 5: посылки про покрытие ответа
        Management API целым vhost'ом или про переиспользование одного ответа
        всеми `conn_id` одного брокера в readme нет вовсе (кэш в них не
        описывается); про то, как завершающаяся consumer-таска решает, отменили
        её или нет, тоже ничего не сказано; про запись статуса сказано только про
        строку `conn_id` (`readme.md:764`, `787` и их ru-близнецы `763`, `786`) —
        это другой механизм, `_StatusWriter` в readme не описан; утверждение про
        fire-консьюмера («уходит вместе с соединением, потому что держит выданный
        при старте объект соединения всю жизнь», `readme.md:764` / `readme_ru.md:763`)
        коду соответствует — `_recover_dead_consumers` отменяет fire-таску вместе
        с пересозданием её `conn_id`, а паузы перед переподпиской readme не
        описывает ни для одного из двух циклов
      - формулировка слоя 4 («публикация, которая продолжает падать, выдерживает
        растущую паузу перед переподключением подписки») описывает ровно то
        поведение, которое задача 1 и обеспечивает
- [x] сверить `docs/adr/0007-connection-liveness-two-tier-check.md` целиком —
...
- [x] переместить этот план в `docs/plans/completed/`
      - перемещение выполняет харнесс после завершения всех фаз; на диске файл
        этой задачей не трогается, иначе последующие фазы, читающие план по
        пути, его не найдут
...
## Что нашли фазы ревью после этих одиннадцати задач

Одиннадцать задач выше — это правки по одному аудиту. После них по ветке прошли
одиннадцать раундов ревью, и они подтвердили и закрыли **26 дефектов уровня
CRITICAL/MAJOR сверх плана**, изменив в пять с лишним раз больше кода, чем сами
задачи. Ни один из этих дефектов не воспроизводится в `main`: символы, вокруг
которых они возникали — `_judge_candidates`, `_probe_consumers`,
`_partition_candidates`, `_StatusWriter`, `_write_conn_status_rows`,
`_cancelled_by_broker`, `_consumer_cache`, `_raised_while_cancelling` — в `main`
отсутствуют. То есть это не разрастание скоупа, а собственная функциональность
ветки, не прошедшая проверку с первого раза.

### Последовательность, в которой правка порождала следующий дефект

Каждая строка: что добавил предыдущий раунд → что нашёл следующий.

1. `9711039` → `2fc8370`. Метка отказа доставки писалась в **объект
   исключения**; `aiormq` раздаёт один и тот же экземпляр всем ожидающим
   вызовам соединения, поэтому метка пересекала кадры и настоящая отмена
   читалась как транзиентный отказ. Заменено на локальный флаг кадра.
2. `a70382c` → `caf4dcb`. Правило «любая идущая запись = не улажено» повторно
   коммитило одинаковый `listening` на каждую доставку (замерено: 100 коммитов
   против 2), а флаш статусов при остановке стоял позади закрытий соединений и
   до него не доходило управление. Введено сравнение с парой, которую
   коммитит идущая запись; флаш перенесён вперёд.
3. `caf4dcb` → `34b1a7f`. Бюджеты остановки не учитывали последовательные
   закрытия по `_CLOSE_TIMEOUT` на каждую роль: двух неотвечающих conn_id
   хватало, чтобы выйти за `_STOP_TIMEOUT`. Закрытия объединены в один
   `asyncio.gather`.
4. `34b1a7f` → `41dadeb`. Guard зелёного статуса читал **локальную** карту
   consumer'ов aiormq, поэтому при полуоткрытой потере связи доставка
   перезаписывала вердикт брокера; а замок вокруг записи conn-статуса брался
   блокирующе **внутри воркера пула** и при зависшей транзакции выедал весь
   четырёхворкерный пул. Введён `denied_by_broker()`; замок стал неблокирующим
   с отбрасыванием устаревшей записи.
5. `41dadeb` → `3d5ee2f`. Теги выбирались до `await`, а ответ применялся к
   текущему тегу: переподключение во время пробы превращало ответ про тег A в
   залипший отказ тега B. Введено `expected: {тег → соединение}` и сравнение
   выбранного тега после пробы.
6. `3d5ee2f` → `0f17a68`. Сравнение ловило смену тега только **после** выборки,
   а кэш Management API отдаёт один снимок всем conn_id той же учётной записи,
   поэтому ответ мог быть старше самой выборки. Снимок получил отметку
   `taken_at`, появился `_answer_misses()`.
7. `0f17a68` → `8e614e5`. У подписки между двумя подключениями `consumer_tag`
   равен `None` при статусе `listening`; `None != None` ложно, отметка стара —
   ответ проходил, и «нет `None` в списке» читалось как отказ. Введён
   `_still_attached()` как единственное определение кандидатности.

### Что у этой последовательности общего

Код судит об удалённом факте, который меняется, пока о нём спрашивают: какой
тег держит брокер, на каком объекте соединения, в какой момент. Три свойства
делали это неисправимым по одной правке за раз:

- **Ответ не нёс, о чём он.** Проба возвращала голое множество тегов, и каждый
  раунд добавлял вызывающему ещё один внешний квалификатор, который надо было
  сопоставить вручную. Прекратилось это только когда данные начали нести своё
  происхождение — `expected: {тег → соединение}` плюс `taken_at`.
- **Отсутствие данных по умолчанию превращалось в вердикт.** «Тега нет в
  ответе» означало отказ, «пробе нечего спросить» — подтверждение. Четыре
  дефекта из семи — буквально это, и содержание последней правки состоит во
  введении третьего исхода: вердикта нет, счётчики не двигаются, строка не
  пишется.
- **Суждение перешагивает через `await`.** Состояние, снятое до
  `await self._probe_consumers(...)`, применяется после него, в функции,
  состояние которой параллельно меняют reconcile, переподключение и замена
  объекта в пуле. Инвариант «ниже этого места нет `await`» держится
  комментарием, а не структурой.

Измеримый симптом: `_judge_candidates` вырос со 101 строки после одиннадцати
задач до 199 к концу ревью. Каждый раунд дописывал в ту же функцию ещё одно
уточняющее условие; перестроена она не была ни разу.

### Что осталось открытым по итогам ревью

- **Правка `8e614e5` не проходила независимого ревью** — прогон закончился на
  ней. Критерий «прогнать ещё раунд, прежде чем считать ветку закрытой» не
  выполнен.
- **У дефекта №1 нет регрессионного теста.** Правка `2fc8370` удалила
  единственное утверждение, отличавшее метку на объекте исключения от
  локального флага, и не поставила ничего взамен: снятие флага тесты роняет,
  возврат к метке — нет.
- **Путь `ConnectionError(...) from None` в `_connect_pooled` не покрыт
  сквозным тестом** — он закреплён только рукотворной репликой формы исключения.
- **`8e614e5` не отражён в ADR-0007**: ни `_still_attached`, ни правило «проба,
  которую ничего не спросили, ничего не подтверждает» там не описаны.
- Консолидация тестовых хелперов сознательно не делалась и за прогон выросла:
  32 повторения `def write(` и 19 копий `_never_returns` в файле на 9184 строки.
- `_judge_candidates` (199 строк) и `_consume_subscription` (127 строк) никем
  не предлагались к перестройке.
