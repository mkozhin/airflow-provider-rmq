...
- [ ] часть A: дать телу `async for` (`consumer.py:2402-2413`) собственный
...
- [ ] часть A: то же для тела доставки fire-цикла (`_consume_fire_queue`,
      ...
      снова
- [ ] часть B: в `_raised_while_cancelling` (`consumer.py:574`) переходить по
      ...
      docstring
- [ ] дополнить docstring функции тем, что именно читается в цепочке и почему
      этого достаточно на 3.10 (без упоминания прежнего поведения)
- [ ] тест: форма 2 — `ConnectionError(...) from None`, поднятый внутри
      `except asyncio.CancelledError`, отменой не считается
- [ ] тест: форма 2 — `asyncio.TimeoutError() from None` из `call_with_timeout`
      в той же позиции отменой не считается
- [ ] тест: форма 3 — `AMQPConnectionError`, поднятый как
      `raise X from CancelledError` (форма `aiormq.TaskWrapper`), отменой не
      считается
- [ ] тест-страж: форма 1 — чужое исключение внутри
...
- [ ] тест-страж: форма 4 — сам `CancelledError` — распознаётся входом по
      `isinstance`, не доходя до обхода цепочки
- [ ] тест-страж: форма 5 — отказ `basic.cancel` внутри `__anext__`, пришедший
...
- [ ] тест на сквозной путь: **cooldown**-подписка, у которой публикация
...
- [ ] тест на сквозной путь: **cooldown**-подписка, у которой publish-соединение
      разорвано во время RPC внутри `_get_publish_channel` (форма 3),
      перезапускается, а не завершается молча
- [ ] тест на сквозной путь: **cooldown**-подписка, у которой пересоздание
      publish-соединения упёрлось в `_DB_TIMEOUT` чтения Airflow-connection,
      перезапускается, а не завершается молча
- [ ] тест на сквозной путь: подписка, у которой сам `__anext__` поднял форму 1,
      завершается как отменённая — путь, который остаётся под эвристикой после
      части A
- [ ] уточнить существующий пункт `CHANGELOG.md:47` (эвристика отмены) вместо
      добавления нового
- [ ] снять часть A: краснеют сквозные тесты на форму 3 и на `_DB_TIMEOUT` —
      их ловит только структурная граница, эвристика на этих формах отвечает
      «отмена»; вернуть
- [ ] снять часть B: краснеют юнит-тесты на форму 2; сквозной тест на таймаут
      публикации при этом остаётся зелёным — после части A он от эвристики не
      зависит, и пинить им часть B нельзя; вернуть
- [ ] в обоих случаях тесты-стражи, включая форму 5, остаются зелёными
