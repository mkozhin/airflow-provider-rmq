...
- [ ] добавить `await asyncio.sleep(_RECONNECT_DELAY)` в конце тела `try`
...
- [ ] тест: итератор fire-очереди, завершающийся сразу и подряд, не крутит цикл
      без задержки — переподписка происходит после `_RECONNECT_DELAY`
- [ ] тест-страж: отмена fire-таски во время этой паузы завершает её штатно
- [ ] уточнить существующий пункт `CHANGELOG.md:44` — он описывает ту же паузу
      для подписочного цикла и после этой правки относится к обоим
- [ ] снять правку: краснеет тест на задержку, тест-страж остаётся зелёным;
      вернуть
...
- [ ] в конце `_store_unwritten_statuses` (`consumer.py:982`) пройти снимок
...
- [ ] убрать ставший избыточным отдельный обход `self._active`: его писатели
      входят в снимок реестра
- [ ] логировать отказ дожима и продолжать цикл, как это делает
      `_ConsumerState.flush` (`consumer.py:686-706`)
- [ ] объяснить в docstring, почему проход идёт по реестру, а не по активным
      подпискам: строка исчезнувшей подписки переживает её саму, и дописать её
      больше некому
- [ ] тест: подписка удалена, запись `disconnected` не прошла — следующий цикл
      дописывает её статус
- [ ] тест: две неудачи подряд не теряют статус — третий цикл всё ещё дожимает
- [ ] тест: отмена цикла на записи `_SUB_DISCONNECTED` не теряет статус —
      следующий цикл его дописывает
- [ ] тест: проход не ломается на подписке, строки которой в БД уже нет
- [ ] тест-страж: писатель, у которого нечего дожимать, обращения к БД не
      вызывает
- [ ] тест-страж: дожим не запускает вторую параллельную запись, когда первая
      ещё идёт
- [ ] уточнить существующий пункт `CHANGELOG.md:49` — он описывает дожим
      незаписанного статуса и после этой правки относится и к подписке, которой
      уже нет
- [ ] снять проход по реестру, убедиться, что краснеют первые три теста и не
...
- [ ] исправить комментарий у `except (TypeError, ValueError, OverflowError)`
...
- [ ] исправить ту же формулировку в `CHANGELOG.md:56`
- [ ] заменить в `docs/backlog.md:144` ссылку на несуществующий `_NACK_SLEEP` на
      реальное место: пауза 0.1 с внутри `nack_and_sleep` (`utils/amqp.py:290`)
- [ ] решить по каждому пункту `**Fixed:**` отдельно, а не диапазоном: сворачивать
      в `**Added:**` только те, чьё поведение в `main` отсутствует. Проверять
      через `git show main:<файл>`, а не по номеру строки
- [ ] явно оставить как `**Fixed:**` пункты, воспроизводимые в выпущенной
...
- [ ] запись: нерезолвящиеся аргументы декоратора молча становятся дефолтами
...
- [ ] запись: замена `self._stop_event` в `_start()` (`listener.py:722`) сразу
      ...
      несведённое событие
- [ ] запись: две ветки `_provision_one_exchange_sub` (`consumer.py:1596` и
...
- [ ] запись: реестр `_status_writers` (`consumer.py:365`) растёт всю жизнь
...
- [ ] в каждой записи указать, воспроизводится ли находка в `main`, и почему
