...
`queue_declare` по соединению. Два подряд отрицательных вердикта пересоздают соединение
`conn_id` вместе со всеми его тасками; недоступность Management API — это «нет данных», а не
вердикт. Судит о живости именно брокер: незавершённая consumer-таска не доказывает ничего.
