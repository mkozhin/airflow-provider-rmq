...
**Unknown**:
Статус `conn_id`, у которого проверка живости ещё ни разу не дошла до вердикта. Это
отсутствие ответа, а не отрицательный ответ.
_Avoid_: error, disconnected (оба означали бы, что о соединении что-то известно)

**Warmup**:
Прогрев: промежуток после старта шедулера, пока DAG-процессор не наполнил `serialized_dag`.
Триггер отвечает `DagNotFound`, хотя соединение, канал и регистрация consumer'а целы.
Доставка возвращается в очередь и ждёт, строка подписки остаётся `listening`. Ограничен: 25
таких доставок подряд читаются как отказ, и строка получает `error`.
_Avoid_: отказ триггера, `error` (прогрев — это ожидание остального Airflow, а не поломка
подписки; отличается и от паузы DAG'а, которая даёт терминальный ACK)

**Op access grant**:
Выдача прав странице подписок: плагин правит права роли Op из отложенного колбэка блюпринта
на старте веб-сервера. Фиксированные списки `sync_roles` не содержат ресурсов стороннего
плагина, поэтому страница иначе достаётся одному Admin. Опция конфигурации Airflow
`[rmq_watcher] grant_op_access` управляет доступом, а не выдачей: права снимает любое написание
отрицания (`false`, `off`, `no`, `n`, `0`), и оно же — написание, которого читатель не знает:
переключатель отбирает привилегию, поэтому непрочитанный ответ читается в сторону отказа. Переключатель живёт в конфигурации, а не в Variable,
потому что Variables роль Op пишет сама и вернула бы себе страницу, закрытую администратором.
_Avoid_: миграция, разовый скрипт (права переустанавливаются на каждом старте, поэтому обновление
провайдера открывает страницу само)
