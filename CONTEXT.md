...
плагина, поэтому страница иначе достаётся одному Admin. Airflow-переменная
`rmq_watcher_grant_op_access` управляет доступом, а не выдачей: ложное значение права снимает,
а нечитаемая переменная не делает ни того, ни другого.
