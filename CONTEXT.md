...
**Прогрев** (warmup):
Промежуток после старта шедулера, пока DAG-процессор не наполнил `serialized_dag`: триггер
отвечает `DagNotFound`, хотя соединение, канал и регистрация consumer'а целы. Доставка
возвращается в очередь и ждёт, строка подписки остаётся `listening`. Ограничен: 25 таких
доставок подряд читаются как отказ, и строка получает `error`.
...
**Выдача прав странице подписок**:
Плагин правит права роли Op из отложенного колбэка блюпринта на старте веб-сервера: фиксированные
списки `sync_roles` не содержат ресурсов стороннего плагина, поэтому страница иначе достаётся
одному Admin. Airflow-переменная `rmq_watcher_grant_op_access` управляет доступом, а не выдачей:
ложное значение права снимает, а нечитаемая переменная не делает ни того, ни другого.
