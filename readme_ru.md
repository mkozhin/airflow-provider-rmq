...
**4 — Публикация на собственном соединении.** Cooldown-публикация идёт по отдельному, лениво поднимаемому соединению на каждый `conn_id`. Под memory- или disk-alarm RabbitMQ блокирует публикующие соединения тем, что перестаёт читать из сокета, — иначе вместе с публикацией встали бы подтверждения всех потребителей этого же соединения; вдобавок соединение, помеченное брокером как заблокированное, перестаёт писать и на стороне клиента (`aiormq` придерживает собственную запись до `connection.unblocked`), поэтому именно разведение оставляет `basic.ack` в движении: потребляющее соединение не публикует ничего и пометки не получает. Чего разведение не покрывает — соединение, поднятое *во время* алярма: годным оно станет только после его снятия, ради чего в первом слое и стоит клиентская граница на connect. Плейсхолдер, который брокер вернул как немаршрутизируемый — pending-очереди нет, и запускать DAG после окна будет некому, — возвращает доставку в очередь, а не подтверждает её. Два подряд таймаута публикации пересоздают только публикующее соединение и не задевают потребление, а публикация, которая продолжает падать, выдерживает растущую паузу (1 с с удвоением до 60 с) перед переподключением подписки — иначе alarm, живущий дольше лимита доставок quorum-очереди, заканчивается тем, что брокер выбрасывает возвращённое в очередь сообщение. Fire-консьюмер cooldown работает на наименьшем `conn_id` среди cooldown-подписок, что не даёт порядку строк из БД перебрасывать его с брокера на брокер; cooldown-подписки, разложенные по нескольким `conn_id`, логируются как ERROR — pending-очереди живут только на одном брокере.
