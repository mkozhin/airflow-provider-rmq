...
Чтобы AST-скан `dag_file`-подписок нашёл `dag_id` в `@dag(...)` (позиционный или именованный аргумент), он должен быть строковым литералом либо простой модульной строковой константой, определённой раньше декорируемой функции. Если это не так, subscription просто не появляется на странице Subscriptions вовсе — не «с бейджем», а полностью отсутствует. Единственный сигнал — WARNING в логе Scheduler'а, и то лишь после рестарта или при изменении mtime файла (не на каждом reconcile-цикле — только когда файл реально пересканируется).
