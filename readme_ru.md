...
> **Предупреждение.** В эти права входят создание и удаление подписок, то есть доступ к странице — это право заставить произвольный DAG запускаться по сообщению из очереди. Установка, где роли Op такое право не положено, ставит Airflow Variable `rmq_watcher_grant_op_access` в `false`.

Переменная управляет доступом, а не только выдачей: ложное значение **снимает** те же шесть прав с роли Op на следующем старте веб-сервера. Выдавать или отбирать их руками при истинном или незаданном значении бессмысленно — старт вернёт ответ провайдера на место.
...
| `rmq_watcher_grant_op_access` | `true` | Держит ли роль Op права [страницы подписок](#доступ-к-странице-подписок) |

Первые две перечитываются в начале каждого цикла в отдельном пуле потоков под собственным коротким таймаутом, так что изменённая переменная вступает в силу со следующего цикла; чтение, всё ещё занимающее воркер, не даёт начаться следующему, а по собственному таймауту петля его бросает и остаётся при уже действующих значениях, а не подвешивается; БД, которая на чтение отвечает отказом, неотличима от незаданной переменной — читается «значения нет», и в силу вступают встроенные умолчания, пока следующий цикл не прочтёт настоящее. Третья читается один раз за старт веб-сервера, когда выставляются права страницы. Читается она раньше, чем что-либо трогается, и БД, которая на неё не ответила, оставляет права ровно как есть — ничего не выдав и ничего не сняв, — с предупреждением в логе: нечитаемый переключатель не решает вопрос доступа.
