...
**4 — Публикация на собственном соединении.** Cooldown-публикация идёт по отдельному, лениво поднимаемому соединению на каждый `conn_id`. Под memory- или disk-alarm RabbitMQ блокирует публикующие соединения тем, что перестаёт читать из сокета, — иначе вместе с публикацией встали бы подтверждения всех потребителей этого же соединения. Плейсхолдер, который брокер вернул как немаршрутизируемый — pending-очереди нет, и запускать DAG после окна будет некому, — возвращает доставку в очередь, а не подтверждает её. Два подряд таймаута публикации пересоздают только публикующее соединение и не задевают потребление, а публикация, которая продолжает падать, выдерживает растущую паузу (1 с с удвоением до 60 с) перед переподключением подписки — иначе alarm, живущий дольше лимита доставок quorum-очереди, заканчивается тем, что брокер выбрасывает возвращённое в очередь сообщение. Fire-консьюмер cooldown работает на наименьшем `conn_id` среди cooldown-подписок, что не даёт порядку строк из БД перебрасывать его с брокера на брокер; cooldown-подписки, разложенные по нескольким `conn_id`, логируются как ERROR — pending-очереди живут только на одном брокере.
