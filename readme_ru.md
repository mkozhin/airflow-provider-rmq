...
- **Единственный отказ триггера, о котором статус молчит:** DAG, у которого Airflow ещё не держит сериализованной версии, — `DagNotFound` из `trigger_dag`, то самое состояние, в котором только что запустившийся шедулер находится, пока DAG-процессор не разобрал файлы. Соединение, канал и регистрация consumer'а целы, и доставка ждёт остальной Airflow, поэтому в строку не пишется ничего: подписка, бывшая в `listening`, в нём и остаётся, а `error`, оставленный более ранним, не связанным отказом, держится до первой прошедшей доставки. Сама доставка при этом возвращается в очередь и выдерживает ту же растущую паузу. Молчание ограничено: 25 таких доставок подряд — уже отказ сам по себе, и строка получает `error` с причиной. Счётчик меряет один непрерывный прогрев — его начинают заново и переподключение, и любая доставка, которая прошла или упала по другой причине, — а каждая десятая несериализованная доставка пишется WARNING'ом, и на quorum-очереди, которая исчерпает `delivery-limit` раньше счётчика, это единственный след прогрева. Fire-консьюмер cooldown поступает с тем же ответом так же и статуса не пишет вовсе: собственной строки у него нет, а один такой консьюмер обслуживает все cooldown-DAG'и, поэтому его события считаются по `dag_id` только ради уровня записи в логе — каждое десятое идёт WARNING'ом. Cooldown-DAG, который не сериализуется никогда, виден поэтому только в логе шедулера.
