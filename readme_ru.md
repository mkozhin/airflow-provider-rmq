...
**4 — Публикация на собственном соединении.** Cooldown-публикация идёт по отдельному, лениво поднимаемому соединению на каждый `conn_id`. Под memory- или disk-alarm RabbitMQ блокирует публикующие соединения тем, что перестаёт читать из сокета, — иначе вместе с публикацией встали бы подтверждения всех потребителей этого же соединения. Два подряд таймаута публикации пересоздают только публикующее соединение и не задевают потребление, а публикация, которая продолжает падать, выдерживает растущую паузу (1 с с удвоением до 60 с) перед переподключением подписки — иначе alarm, живущий дольше лимита доставок quorum-очереди, заканчивается тем, что брокер выбрасывает возвращённое в очередь сообщение. Fire-консьюмер cooldown работает на наименьшем `conn_id` среди cooldown-подписок, что не даёт порядку строк из БД перебрасывать его с брокера на брокер; cooldown-подписки, разложенные по нескольким `conn_id`, логируются как ERROR — pending-очереди живут только на одном брокере.
...
| Колонка | Значение |
|---|---|
| Status | `connected` — брокер подтвердил все наши consumer-теги на этом `conn_id`; `error` — отрицательный вердикт: consumer, которого брокер не подтвердил, полное отсутствие работающих тасок этого `conn_id` либо неудачная попытка соединения (причина в Last Error); `degraded` — отрицательный вердикт задержан ограничителем частоты пересозданий, и соединение оставлено на месте вместо очередного пересоздания; `unknown` — по этому `conn_id` ещё ни одна проверка не выносила вердикта; `disconnected` — значение колонки по умолчанию, которое строка носит до первого записанного вердикта. Статус идёт только за вердиктом: без вердикта строка сохраняет записанное в ней значение, а число запущенных watcher'ом тасок не красит её в зелёный никогда |
