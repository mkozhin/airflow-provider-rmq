...
Первые две перечитываются в начале каждого цикла в отдельном пуле потоков под собственным коротким таймаутом, так что изменённая переменная вступает в силу со следующего цикла; чтение, всё ещё занимающее воркер, не даёт начаться следующему, а недоступная БД оставляет в силе последние известные значения, а не подвешивает петлю. Третья читается один раз за старт веб-сервера, когда выставляются права страницы. Читается она раньше, чем что-либо трогается, и БД, которая на неё не ответила, оставляет права ровно как есть — ничего не выдав и ничего не сняв, — с предупреждением в логе: нечитаемый переключатель не решает вопрос доступа.
