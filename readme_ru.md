...
|---|---|---|---|
| `heartbeat` | `30` на асинхронном пути, `600` в `RMQHook` | любое AMQP-соединение | Секунды между heartbeat-кадрами AMQP. Клиент считает связь мёртвой после двух пропущенных интервалов, поэтому разрыв превращается в исключение примерно за `2 × heartbeat` секунд, и `connect_robust` переподключается |
...
**2 — Watchdog живости подписок.** Каждый reconcile-цикл watcher спрашивает у брокера, зарегистрированы ли ещё его consumer'ы. Каждая подписка подписывается под детерминированным тегом `rmq_watcher.{hostname}.{pid}.{sub_id}` (у fire-консьюмера cooldown вместо `{sub_id}` стоит `fire`), и проверка ищет именно этот тег — чужой consumer на той же очереди, второй DAG, подписанный на неё, или вторая реплика scheduler'а в HA за нашего поручиться не могут. Если задан `management_url`, тег ищется в ответе `GET /api/consumers/{vhost}`; если нет — watcher делает passive `queue_declare` на отдельном канале, и его вердикт распространяется на все подписки этого `conn_id`. Две подряд отрицательные проверки (то есть не раньше двух reconcile-интервалов) отменяют таски этого `conn_id`, сбрасывают соединение и поднимают таски заново — перезапуск одной только таски выдал бы ей из пула то же самое зомби-соединение. Недоступность Management API — это «нет данных», а не вердикт: WARNING в лог, счётчики не трогаются, сохранённый статус остаётся. А вот зависшая AMQP-проба считается признаком смерти. Один и тот же `conn_id` не пересоздаётся чаще, чем раз в 5 циклов; вердикт, задержанный этим ограничителем, пишется в лог и отражается статусом `degraded`.
...
Обе перечитываются раз в несколько циклов в отдельном пуле потоков под собственным коротким таймаутом; недоступная БД оставляет в силе последние известные значения, а не подвешивает петлю.
...
| Колонка | Значение |
|---|---|
| Status | `connected` — брокер подтвердил наших consumer'ов, `connecting` — таска ещё поднимается, `error` — отрицательный вердикт (причина в Last Error), `degraded` — отрицательный вердикт задержан ограничителем частоты пересозданий |
| Consumers | Сколько consumer-тасок запустил сам watcher |
| Broker consumers | Сколько consumer'ов видит на наших очередях брокер; `—` означает, что проверка не дала данных. Число, отличающееся от предыдущей колонки, подсвечивается: меньше — есть подписка, которой брокер не видит, больше — на тех же очередях сидят другие клиенты или вторая реплика scheduler'а |
...
Строка пишется для **каждого** `conn_id` из списка подписок, включая тот, у которого умерли все таски, — иначе мёртвый `conn_id` просто замер бы на последнем значении.

**Миграция схемы.** В таблице статусов появились две колонки: `broker_consumer_count` и `last_reconcile_at`. Миграция идемпотентна: она смотрит на живую таблицу и выполняет `ALTER TABLE … ADD COLUMN` только для отсутствующего. Плагин запускает её один раз при загрузке, а петля watcher'а повторяет в начале каждого цикла (единственная попытка в полёте, с растущим backoff'ом между неудачами), пока миграция не пройдёт, — так что недоступная в момент загрузки БД не оставляет процесс работать против устаревшей таблицы. Пока таблица отстаёт, блок Connections показывает короткое уведомление вместо ошибки рендеринга страницы.
